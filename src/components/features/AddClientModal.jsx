...
import { createClient } from '@/firebase/firestore'
...
const INITIAL_STATE = {
  statusTag: 'New',
  personal: {
    firstName: '', lastName: '', gender: 'Female', dob: '', country: 'India',
    city: '', heightCm: '', email: '', phone: '', religion: 'Hindu', caste: '',
    motherTongue: 'Hindi', manglik: 'Dont Know', maritalStatus: 'Never Married',
    complexion: 'Fair', dietaryPref: 'Vegetarian', smoking: 'Never',
    drinking: 'Never', languagesKnown: '', photoUrl: ''
  },
  professional: {
    college: '', degree: '', company: '', designation: '',
    annualIncomeLakh: '', nriStatus: false, workCity: ''
  },
  preferences: {
    wantKids: 'Maybe', openToRelocate: 'Maybe', openToPets: 'Maybe',
    partnerAgeMin: '', partnerAgeMax: '', partnerHeightMinCm: '',
    partnerIncomeMinLakh: ''
  },
  family: {
    fatherOccupation: '', motherOccupation: '', brothers: '', sisters: '',
    familyType: 'Nuclear', familyIncomeLakh: '', familyValues: 'Moderate'
  }
}
...
export default function AddClientModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newClient = await createClient(user.uid, cleanData)
      
      // Optimistic UI update
      setClients([newClient, ...clients])
      
      addToast({ type: 'success', title: 'Client Created', message: `${newClient.personal.firstName} added successfully.` })
      onClose()
    } catch (err) {
      addToast({ type: 'error', title: 'Error', message: 'Failed to create client.' })
    } finally {
      setIsSubmitting(false)
    }
  }

  // Common input renderer
    ...
    if (type === 'checkbox') {
      return (
        <label className="flex items-center gap-2 mt-6 cursor-pointer">
          <input type="checkbox" checked={value} onChange={onChange} className="w-4 h-4 text-brand rounded border-surface-300 focus:ring-brand" />
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add New Client" hideCloseButton={isSubmitting}>
      
      {/* Progress Tabs */}
      <div className="flex items-center mb-6">
        {[1, 2, 3].map((num) => (
          <React.Fragment key={num}>
            <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${step >= num ? 'bg-brand text-white' : 'bg-surface-100 text-surface-400'}`}>
              {num}
            </div>
            {num < 3 && (
        ...
        {step === 3 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="mt-6 p-4 bg-surface-50 border border-surface-200 rounded-lg">
              <label className="block text-xs font-semibold text-surface-600 mb-1">Status on Creation</label>
              <select 
